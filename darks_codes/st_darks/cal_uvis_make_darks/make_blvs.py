...
def buildasntable(proc_dir):
    ...
    logging.info('\tBuilding the association table.')
...
def run_calwf3():
    ...
    logging.info('\tRunning CALWFC3 to create blv_tmp.fits files.')

    # Set iref environment
    os.environ['iref'] = '/grp/hst/cdbs/iref/'

    # Run calwf3, which will make blv_tmp.fits files in the directory
    calwf3(input='dark_asn.fits', save_tmp=True)


def make_blvs_main(proc_dir):
    ...
    logging.info('')
    logging.info('')
    logging.info('---------- Making blv_tmp.fits files for {} ----------'.format(proc_dir_short))
    logging.info('')

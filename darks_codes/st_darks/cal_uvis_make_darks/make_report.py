    ...
    for data_type in ['blvs', 'superdarks']:
        for plot_type in ['midpt', 'hotpix']:
            for ext in ['1', '4']:
                ...
                logging.info('\tCopied {} to {}'.format(src, dst))
...
def create_imstat_files(paths):
    ...
    for ext in [1, 4]:
        logging.info('')
        logging.info('\tGathering extension {} statistics.'.format(ext))
        ...
        logging.info('\t\tstatistics written to {}'.format(os.path.join(paths['report_dir'], 'imstat_ext{}.dat'.format(ext))))
...
def make_jpeg(frame, paths):
    ...
    for ext in [1, 4]:
        ...
        logging.info('\t\tSaved JPEG to {}'.format(jpeg_output))
...
def make_report_main(paths, ctecorr):
    ...
    logging.info('')
    logging.info('')
    logging.info('---------- Making report directory ----------')
    logging.info('')

    # Create report directory
    os.mkdir(paths['report_dir'], 0o774)
    logging.info('\tCreated directory {}'.format(paths['report_dir']))
    logging.info('')

...
def grow_cr_mask_main(crrej_dir):
    ...
    print('')
    print('')
    print('---------- Growing cosmic ray mask for {} ----------'.format(crrej_dir.split('/')[-1]))
    print('')

    # Prepare crgrow directory
    crgrow_dir = os.path.join(crrej_dir, 'crgrow/')
    os.mkdir(crgrow_dir, 0o774)
    print('\tCreated directory {}'.format(crgrow_dir))
    print('')
    ...
    if len(blv_list) > 0:
        for blv_file in blv_list:
            ...
            print('\tMoved {} to {}'.format(blv_file, crgrow_dir))
        print('')
        ...
        print('\tNo data for {}'.format(crrej_dir))

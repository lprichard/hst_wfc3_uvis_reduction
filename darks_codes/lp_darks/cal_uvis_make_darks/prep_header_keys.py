...
def set_keywords(image_list, paths, postflash):
    ...
    print('\tSetting header keywords for {}'.format(os.path.dirname(image_list[0])))
    ...
    for image in image_list:
        ...
        if 'crrej' in image:
            header['CRREJTAB'] = r'calib_dir$' + paths['hotpix_crrej'].split("/")[-1]   #LP edit due to truncation issues
        else:
            header['CRREJTAB'] = r'calib_dir$' + paths['dark_crrej'].split("/")[-1]     #LP edit due to truncation issues
...
def prep_header_keys_main(paths, postflash):
    ...
    print('')
    print('')
    print('---------- Preparing Header Keywords ----------')
    print('')

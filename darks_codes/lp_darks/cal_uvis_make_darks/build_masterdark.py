...
from cal_uvis_make_darks.replace_good_pixels import replace_good_pixels_main
...
def build_masterdark_main(paths, anneal_date, ctecorr, mode):
    ...
    if not os.path.exists(os.path.dirname(paths['masterdark_create_dir'])): 
        os.makedirs(os.path.dirname(paths['masterdark_create_dir']), 0o774)
        print('\tLP log: Created driectory {}'.format(os.path.dirname(paths['masterdark_create_dir'])))

    # Make masterdark directory
    os.mkdir(paths['masterdark_create_dir'], 0o774)
    print('\tCreated directory {}'.format(paths['masterdark_create_dir']))
    print('')
    ...
    for nnblv in nnblvs_list:
        ...
        print('\tCopied {} to {}'.format(nnblv, paths['masterdark_create_dir']))
    print('')
    for superdark in superdark_list:
        shutil.copy(superdark, paths['masterdark_create_dir'])
        print('\tCopied {} to {}'.format(superdark, paths['masterdark_create_dir']))
    print('')
    ...
    # os.chown(masterdark, -1, 340)   #LP comment, this is for WFC3 related permissions
